...
def enter_move(board):
	...
	while not ok:
		...
		if not ok:
			print("Zły ruch - wykonaj ruch ponownie!") # no, it isn't - do the input again
			continue
		...
		if not ok:	# it's occupied - to the input again
			print("Pole zajęte - wykonaj ruch ponownie!")
			continue
...
def victory_for(board, sgn):
	...
	for rc in range(3):
		...
		if board[rc][rc] != sgn: # check the first diagonal
			cross1 = False
		if board[2 - rc][2 - rc] != sgn: # check the second diagonal
			cross2 = False
	if cross1 or cross2:
		return who
	return None
